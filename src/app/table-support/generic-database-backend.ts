...
export class GenericDatabaseBackend<T extends IId> implements GenericDatabaseInterface<T> {
  ...
  public select(start: number,
    ...
    const cache = this.cache.getRange(start, length, filter, order, this.includedTypesAtSelect);
    if (!isUndefined(cache)) {
      subject.next(cache);
    } else {
      this.rest.getRange(start, length, filter, this.filterColumns, order, this.includedTypesAtSelect).subscribe((result) => {
